...
YOLO-модель интегрирована как кастомная Ultralytics-модель для локального offline-инференса. Пайплайн передает координатную сетку детекций на backend, но текущие веса лучше всего рассматривать как MVP-версию: на сложных схемах путепроводов, мостовых разрезах и чертежах без табличных ориентиров возможны пропуски балок, опор и свай. Следующий шаг развития - Data-Centric дообучение: отбор целевых схем из архива 3.5 ГБ, первичная авторазметка текущей моделью, корректировка в CVAT/Roboflow и обучение на повышенном входном разрешении.
...
- После исправлений frontend для мобильной версии система стала работать значительно медленнее. Это стоит отдельно проверить профилированием: загрузку списка чертежей, рендер страницы чертежа, работу поиска и WebSocket-обновлений на мобильных разрешениях.
- Поиск похожих чертежей как отдельный пользовательский сценарий пока не реализован. При этом техническая основа уже есть: FAISS/векторная база, эмбеддинги Sentence Transformers, endpoint поиска и выдача top-k результатов. Следующий шаг - оформить это в UI и API как поиск аналогов текущего чертежа, а не только как текстовый поиск по коллекции.
- Текущий `/api/search` сначала обращается к векторному поиску агента, а при отсутствии результатов использует regex-поиск по MongoDB. Такой fallback полезен для MVP, но качество ранжирования и объяснимость результатов стоит улучшить: показывать тип совпадения, score, фрагмент найденного текста и причину попадания в выдачу.
- YOLO-модель уже интегрирована в offline pipeline, но текущие веса лучше рассматривать как MVP. Для сложных схем, разрезов и чертежей без явных табличных ориентиров возможны пропуски конструктивных элементов; приоритетное развитие - доразметка целевых чертежей и дообучение модели на специализированном наборе данных.
- Часть тестов и локальных запусков зависит от Redis, MongoDB, PostgreSQL, локальных моделей и корректных переменных окружения. Для стабильной разработки полезно выделить быстрый smoke-набор, который проверяет основные API без полного тяжелого AI-пайплайна.
- Ручной запуск отдельных сервисов требует внимательного разведения портов: `drawing_agent` и backend в локальном режиме могут конфликтовать за порт `8000`, а Vite proxy должен смотреть именно на backend.
